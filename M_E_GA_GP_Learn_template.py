...
class ExperimentGA(M_E_GA_Base):
    def __init__(self, phase, cycle, experiment_name, fitness_function, *args, **kwargs):
    ...
    def load_encodings(self, encodings):
        self.encoding_manager.integrate_uploaded_encodings(encodings, self.fitness_function.genes)

def run_experiment(experiment_name, num_cycles, phase_sequence, fitness_function):
...
if __name__ == '__main__':
    ...
    fitness_function = MegaGPFitnessFunction(8)  # Input size example
    run_experiment(experiment_name, 3, phase_sequence, fitness_function)
